var tmp = require('tmp');
var fs = require('fs');

const {exec} = require('child_process')

async function compile(req,res)
{
    const {code,input,language} =req.body
    let result = '';
       const extensions = {
        java:".java",
        cpp:".cpp",
        c:".c",
        cs:".cs",
        python:".py",
        javascript:".js"

       }

        tmp.file({prefix: 'project', postfix: extensions[language]},async  function (err, path, fd, cleanupCallback) {
  
            if (err) throw err;
         
       console.log("File: ", path);
       console.log("Filedescriptor: ", fd);
        if(language === 'java')
        {
            fs.rename(path,__dirname+'/Main.java',()=>console.log('renamed')) 
            fs.writeFile('Main.java', code, () => console.log('written file'))
        }
        else
          fs.writeFile(path, code, () => console.log('written file'))
       console.log(__dirname);
    const isWindows = process.platform === "win32";

    const commands = {
    java: `javac Main.java && java Main`,

    cpp: isWindows
        ? `g++ ${path} -o outputCPP.exe && outputCPP.exe`
        : `g++ ${path} -o outputCPP && ./outputCPP`,

    c: isWindows
        ? `gcc ${path} -o outputC.exe && outputC.exe`
        : `gcc ${path} -o outputC && ./outputC`,

    cs: isWindows
        ? `mcs ${path} && ${path.substring(0,path.length-3)}.exe`
        : `mcs ${path} && mono ${path.substring(0,path.length-3)}.exe`,

    python: isWindows
        ? `python ${path}`
        : `python3 ${path}`,

    javascript: `node ${path}`
    };
       const initialTime = Date.now();
       const child = exec(commands[language], { stdio: 'pipe' }, (error, stdout, stderr) => {
        if (error) {
         res.json({output : error.message,runtime : Date.now()-initialTime});
         return;
        }
        res.json({output : stdout,runtime:Date.now()-initialTime});
      });
      child.stdin.write(input);
      if(language === 'java')
      fs.rm('Main.class',()=>console.log('deleted class java'));

      child.stdin.end(); 

      
});
}

module.exports = {
    compile
}